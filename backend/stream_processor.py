...
class RTSPStreamProcessor:
    ...
    def process_stream(self):
        ...
        while self.is_running:
            ...
            try:
                with self.lock:
                    detected, detections = self.detector.detect_vehicles(frame, self.bay_id)
                    ...
                    if detected and detections:
                        ...
                        # Only log significant detection changes
                        logger.debug(f"Bay {self.bay_id}: Car detected (conf: {self.detection_confidence:.2f})")
                    else:
                        self.detection_confidence = 0.0
